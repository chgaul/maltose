...
- All datasets except HOPV are used (i.e., QM9, Alchemy, and OE62), with the
  unified split (split v2). Batch size reduced to 8 in order to run a
  Quadro P4000 GPU.
- B3LYP and PBE0 targets are available (but on disjoint subsets). The model
  trains (HOMO, LUMO, gap) x (B3LYP, PBE0)
- The network has the Set2Set + molecule-level output module with six outputs.
  Otherwise, the architecture is the same multitask_model, multitask_model_v05.
"""
import torch
